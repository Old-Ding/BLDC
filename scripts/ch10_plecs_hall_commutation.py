...
NAMES = (
    ...
    "hall_transition_class","hall_legal_event","hall_direction","hall_enable","hall_fault",
)
...
def main() -> int:
    ...
    try:
        ...
        for scenario in SCENARIOS:
            ...
            if len(values)!=len(NAMES): raise RuntimeError(f"signals={len(values)}")
            ...
            fault_fraction=sum(round(v)>0 for v in data["hall_fault"])/len(times)
            enable_fraction=sum(round(v)>0 for v in data["hall_enable"])/len(times)
            rows.append({"scenario":scenario["name"],"offset_steps":scenario["offset"],"table_direction":scenario["direction"],"final_speed_rad_s":data["speed_rad_s"][-1],"mean_torque_Nm":statistics.fmean(data["torque_Nm"]),"positive_torque_fraction":sum(v>0 for v in data["torque_Nm"])/len(times),"peak_phase_current_A":peak,"hall_fault_fraction":fault_fraction,"hall_enable_fraction":enable_fraction,"result":"MEASURED"})
    ...
    by_name["offset_0"]["result"]="PASS" if float(correct["final_speed_rad_s"])>60 and float(correct["mean_torque_Nm"])>0.8 and float(correct["peak_phase_current_A"])<30 and float(correct["hall_fault_fraction"])==0 and float(correct["hall_enable_fraction"])>0.99 else "FAIL"
    ...
    row=by_name["all_off"];row["result"]="PASS" if abs(float(row["mean_torque_Nm"]))<1e-12 and float(row["peak_phase_current_A"])<1e-12 and float(row["hall_enable_fraction"])==0 else "FAIL"
    with (wave/"plecs_hall_commutation_summary.csv").open("w",encoding="utf-8",newline="") as stream:
        writer=csv.DictWriter(stream,fieldnames=rows[0].keys(),lineterminator="\r\n");writer.writeheader();writer.writerows(rows)
    lines=["# 第 10 章 Hall 换相与安装偏置 PLECS 报告","","| 场景 | 偏置/60°步 | 表方向 | 末值速度/rad/s | 平均转矩/N m | 正转矩占比 | 峰值相电流/A | Hall fault 占比 | Hall enable 占比 | 结果 |","|---|---:|---:|---:|---:|---:|---:|---:|---:|---|"]
    for row in rows:lines.append("| {scenario} | {offset_steps} | {table_direction} | {final_speed_rad_s:.3f} | {mean_torque_Nm:.3f} | {positive_torque_fraction:.3f} | {peak_phase_current_A:.3f} | {hall_fault_fraction:.3f} | {hall_enable_fraction:.3f} | {result} |".format(**row))
    (root/"reports/10-hall-commutation-test_report.md").write_text("\r\n".join(lines)+"\r\n",encoding="utf-8",newline="")
    passed=sum(row["result"]=="PASS" for row in rows);print(f"Generated chapter 10 PLECS Hall commutation evidence. scenarios=8 pass={passed} time_points=401 signals={len(NAMES)} best_offset=0")
