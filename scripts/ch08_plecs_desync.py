...
from ch08_desync_classifier_oracle import annotate_rows
...
SCENARIOS = (
    {"name": "sync_follow", "role": "sync_positive", "f0": 1.0, "f1": 10.0, "ramp": 0.10, "load_after": 0.0, "load_step": 1.0},
    {"name": "gentle_ramp", "role": "mild_desync", "f0": 5.0, "f1": 25.0, "ramp": 0.25, "load_after": 0.0, "load_step": 1.0},
    {"name": "overfast_ramp", "role": "overfast_desync", "f0": 5.0, "f1": 80.0, "ramp": 0.08, "load_after": 0.0, "load_step": 1.0},
    {"name": "load_step", "role": "load_desync", "f0": 5.0, "f1": 25.0, "ramp": 0.25, "load_after": 8.0, "load_step": 0.18},
)
...
def main() -> int:
    ...
    try:
        ...
        for scenario in SCENARIOS:
            ...
            rows.append({
                "scenario": scenario["name"], "role": scenario["role"], "final_frequency_Hz": frequency[-1],
    ...
    for row in rows:
        lines.append(
            "| {scenario} | {role} | {final_frequency_Hz:.3f} | {tail_mean_speed_rad_s:.3f} | {speed_ratio:.4f} | "
            "{phase_slip_cycles:.3f} | {negative_torque_fraction:.3f} | {tail_mean_torque_Nm:.3f} | "
            "{classification} | {result} |".format(**row)
        )
    (root / "reports/08-open-loop-desync-test_report.md").write_text(
        "\r\n".join(lines) + "\r\n", encoding="utf-8", newline=""
    )
    passed = sum(row["result"] == "PASS" for row in rows)
    print(f"Generated chapter 08 PLECS desynchronization evidence. scenarios={len(rows)} pass={passed} time_points=701 signals=15")
